...
export default function Dashboard() {
  const { currentUser, users, documents, meetings, payments, setActiveTab, setSelectedUserProfileId, pipelineStages, selectedCurrency } = useStore();
  ...
  const renderSuperAdminStats = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12 animate-in fade-in slide-in-from-bottom-5">
      ...
      <StatCard icon={TrendingUp} label="Agency Revenue" value={`${selectedCurrency.symbol}${payments.reduce((acc, p) => acc + p.amount, 0).toLocaleString()}`} color="red" trend="+10%" />
    </div>
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12 animate-in fade-in slide-in-from-bottom-5">
        <div className="bg-card p-8 rounded-[2rem] border border-border shadow-xl flex flex-col justify-between transition-all hover:scale-[1.01] active:scale-[0.99] group relative overflow-hidden">
          ...
          <div className="mt-10 space-y-4 relative z-10">
            <div className="flex justify-between text-[11px] font-black text-slate-500 dark:text-slate-400 items-center">
            ...
            <div className="bg-slate-100 dark:bg-slate-800 h-5 rounded-full overflow-hidden p-1 border border-slate-200 dark:border-slate-700 shadow-inner">
